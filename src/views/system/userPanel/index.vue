            ...
            <ul class="list-group list-group-striped">
              ...
              <li class="list-group-item">
                ...
                <div class="pull-right">
                  <div v-if="state.user.volunteerStatus == '2'">是</div>
                  <div v-else-if="state.user.volunteerStatus == '0'">
                    <el-link type="primary" @click="addVolun(state.user)">
                      否（点击加入）
                    </el-link>
                  </div>
                    <div v-else>
                        <dict-tag :options="sys_examine_status" :value="state.user.volunteerStatus"/>
<!--                        <el-link type="primary" @click="addVolun(state.user)">-->
<!--                            否（点击加入）-->
<!--                        </el-link>-->
                    </div>
        ...
        <el-card shadow="never">
          ...
          <span>{{ parseTime(scope.row.startTime, '{y}-{m}-{d}') }}至{{
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template #default="scope">
                <el-button link type="primary" @click="handleAdd(scope.row)" :disabled="state.user.volunteerStatus != '2' || scope.row.userApprovalStatus!='0' ">
                    加入
                </el-button>
...
function handleAdd(row) {
  let data = {
    ...
    volunteerStatus: state.user.volunteerStatus
  }
